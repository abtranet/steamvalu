...
Source: https://www.the reference site/features
...
`src/components/sites/reference-site/shared/*`) did not yet exist in
...
3. **Supported external data sources** (`section` > `.features_support-wrap`)
   ...
   feed reference site RTLS), each with a small checkmark-bulleted list of vendor
   names.
...
## Responsive breakpoints (from the reference site's compiled `site.css`)
