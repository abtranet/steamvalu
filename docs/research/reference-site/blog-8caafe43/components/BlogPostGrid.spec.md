...
## Overview
- **Target files:**
  - `src/components/sites/reference-site/blog-8caafe43/BlogPostGrid.tsx`
    (grid wrapper, maps over post data)
  - `src/components/sites/reference-site/blog-8caafe43/BlogPostCard.tsx`
    (single card — image, title, excerpt, date, button)
  - `src/components/sites/reference-site/blog-8caafe43/data.ts`
    (typed array of the 37 real posts scraped from the live site)
- **Screenshot:** `docs/design-references/reference-site/blog-8caafe43/live-card-grid-800w.jpg`
...
`docs/research/reference-site/blog-8caafe43/posts-data-reference.json`
(also mirrored into the component's `data.ts`). Newest first:
"On-Premise reference site on an Air-Gapped Plant Network" (August 30, 2026) down to
"Pushing the Boundaries" (May 3, 2024). Do not fabricate additional posts —
use exactly these 37.

## Assets
- 37 thumbnail images downloaded to
  `public/sites/reference-site/blog-8caafe43/images/<file>` — referenced
  in `data.ts` as `/sites/reference-site/blog-8caafe43/images/<file>`.
