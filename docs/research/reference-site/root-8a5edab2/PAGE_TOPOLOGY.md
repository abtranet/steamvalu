# Page Topology — the reference site homepage (`/`)

Source: https://www.the reference site/ (fetched live, Sep 2026). Webflow-built, server-rendered HTML + GSAP/ScrollTrigger/Flip for scroll interactions, Lenis for smooth scroll, Swiper for the testimonials carousel.

Full raw HTML saved for reference during build: fetched via `curl` (see section extracts below); screenshots in `docs/design-references/reference-site/root-8a5edab2/`.
...
## Assets referenced (downloaded to `public/sites/reference-site/root-8a5edab2/`)
