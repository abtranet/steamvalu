...
- Reuse whatever Button/pill component pattern the shared Header already established for "Try for free" / "Book Demo" (check `src/components/sites/steamvalu/shared/Header.tsx` for the exact classes before writing new ones, to stay visually consistent with the nav's own CTA buttons).
